pub mod integrate {
    ...
    pub fn midpoint_rule<F>(f_int: F, a: f64, b: f64, n: usize) -> f64
    where
        F: Fn(f64) -> f64,
    {
        let h = (b - a) / (n as f64);
    ...
    pub fn trapezoidal_rule<F>(f_int: F, a: f64, b: f64, n: usize) -> f64
    where
        F: Fn(f64) -> f64,
    {
        let h = (b - a) / (n as f64);
    ...
    pub fn simpsons_rule<F>(f_int: F, a: f64, b: f64, n: usize) -> f64
    where
        F: Fn(f64) -> f64,
    {
        let h = (b - a) / (n as f64);
    ...
    pub fn stopping_criterion<N>(tol: f64, i_numerical: N) -> f64
    where
        N: Fn(usize) -> f64,
    {
        let mut n = 4;
        ...
        loop {
            ...
            result = i_numerical(n);
            if (result - prev_result).abs() <= tol {
                break;
            }
        }
