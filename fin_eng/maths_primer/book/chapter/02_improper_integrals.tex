...
The yield of a 14 months quarterly coupon bond with 8\% coupon rate is 7\%.
Compute the price, duration, and convexity of the bond.

\paragraph{Answer}

The price, duration, and convexity of the bond are as follows
\begin{align*}
    B &= \sum_{i=1}^{4} 2 e^{-\frac{3 i - 1}{12} y} +
        102 e^{-\frac{14}{12} y}; \\
    D &= \frac{1}{B} \sum_{i=1}^{4} \left( \frac{2(3i - 1)}{12}
        e^{-\frac{3 i - 1}{12} y} + 102 \frac{14}{12} e^{-\frac{14}{12} y}
        \right); \\
    D &= \frac{1}{B} \sum_{i=1}^{4} \left( \left( \frac{3i - 1}{12} \right)^2 2
        e^{-\frac{3 i - 1}{12} y} + 102 \left( \frac{14}{12} \right)^2
        e^{-\frac{14}{12} y} \right).
\end{align*}

\subsection{Question 12}
Compute the price, duration, and convexity of a two year semiannual coupon
    bond with face value 100 and coupon rate 8\%, if the zero rate curve is
    given by $ r(0, t) = 0.05 + 0.01 \ln \left( 1 + \frac{t}{2} \right) $.

\paragraph{Answer}

The price, duration, and convexity of the bond are as follows
\begin{align*}
    B &= \sum_{i=1}^{4} c_i \exp \left( -\int_{0}^{t_i} 0.05 +
        0.01 \ln \left( 1 + \frac{\tau}{2} \right) d\tau \right).
\end{align*}
where
\begin{align*}
    \mathbf{c} &=
...
If the coupon rate of a bond goes up, what can be said about the value of the
    bond and its duration?
Give a financial argument.
Check your answer mathematically, i.e., by computing
    $ \frac{\partial B}{\partial C} $ and $ \frac{\partial D}{\partial C} $,
    and show that these functions are either always positive or negative.

\paragraph{Answer}
If the coupon rate of a bond goes up, the value of the bond goes up as the
    coupon payments increase.

The duration of a bond is the weighted time average of the future cash flows
    of the bond discounted with respect to the yield of the bond, and normalized
    by dividing by the price of the bond.
Therefore, as the coupon rate goes up, the duration of a bond goes down as each
    cash flow will be a larger fraction of the payment at maturity.

\subsection{Question 14}
By how much would the price of a ten year zero-coupon bond change if the yield
    increases by ten basis points?
(One percentage point is equal to 100 basis points.
Thus, 10 basis points is equal to 0.001.)

\paragraph{Answer}
The change in price from a increase in yield by ten basis points is
\begin{align*}
    \frac{\Delta B}{B} &= -D \Delta y \\
                       &= -10 * 0.001 \\
                       &= -0.01
\end{align*}
The price of the bond decreases by 1\%.

\paragraph{Question 15}
A five year bond with duration $ 3 \frac{1}{2} $ years is worth 102.
Find an approximate price of the bond if the yield decreases by fifty basis
    points.

\paragraph{Answer}
The change in price from a decrease in yield by fifty basis points is
\begin{align*}
    \frac{\Delta B}{B} &= -D \Delta y \\
                       &= -3.5 * -0.005 * 102 \\
                       &= 1.785 \\
...
\begin{equation*}
    C = D^2 - \frac{\partial D}{\partial y}
\end{equation*}

\paragraph{Answer}
Recall that
\begin{equation*}
    D = -\frac{1}{B} \frac{\partial B}{\partial y} \quad \text{and} \quad
        C = \frac{1}{B} \frac{\partial^2 B}{\partial y^2}.
\end{equation*}
Therefore,
\begin{equation*}
    \frac{\partial B}{\partial y} = -D B
\end{equation*}
Using Product Rule to differentiate with respect to $ y $,
\begin{align*}
    \frac{\partial^2 B}{\partial y^2}
        &= -\frac{\partial D}{\partial y} B - \frac{\partial B}{\partial y} D \\
        &= -\frac{\partial D}{\partial y} B - D(-DB) \\
        &= -B \frac{\partial D}{\partial y} + B D^2 \\
        &= B \left( D^2 - \frac{\partial D}{\partial y} \right) \\
    \frac{1}{B} \frac{\partial^2 B}{\partial y^2} &=
        B \left( D^2 - \frac{\partial D}{\partial y} \right)
\end{align*}
Thus,
\begin{equation*}
